...
* Exercise Description: This program connects LEDs to PB0, PB1, PB2, and PB3. In one state machine (Three LEDs), 
*						output to a shared variable (threeLEDs) the following behavior: set only bit 0 to 1, then 
*						only bit 1, then only bit 2 in sequence for 1 second each. In a second state machine (Blinking LED), 
...
int main()
{
	DDRB = 0xFF; PORTB = 0x00; // Init port threeLEDsto 0s
