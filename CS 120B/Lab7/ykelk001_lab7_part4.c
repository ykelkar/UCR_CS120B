...
*						and output to the PORTB. In another state machine, allow a user to adjust the sound frequency up or down 
*						using buttons connected to PA0 (up) and PA1 (down).
*
...
enum adj_States{Init3, Changer, Inc_Press, Inc_Release, Dec_Press, Dec_Release} adj_state;

unsigned char curr_freq = 0x00;
...
void Tick_Sound()
{
	...
	switch(sound_state)
	{
		...
		case SoundOn:
			if ((time_counter % curr_freq == 0) && (sound_switch == 0x04))
			{
